...
interface ProductTableProps {
  products: Product[];
  loading: boolean;
  // Agregamos la función onDelete como propiedad para que el padre la maneje
            ...
            <tr key={product.id} className="border-b border-gray-800 hover:bg-gray-800/50 transition-colors">
              <td className="px-6 py-4">
                <div className="flex items-center gap-3">
                  {product.images && product.images.length > 0 ? (
                    <div className="relative w-12 h-12 rounded-lg bg-gray-800 border border-gray-700 overflow-hidden shrink-0">
                      <Image 
              ...
              <td className="px-6 py-4 text-right">
                <div className="flex justify-end gap-2">
                  ...
                  {/* BOTÓN EDITAR: Ahora es un Link que te lleva a la ruta del producto */}
                  <Link 
                  ...
                  {/* BOTÓN ELIMINAR: Ahora ejecuta una función con confirmación previa */}
                  <button 
                    onClick={() => {
                      if (window.confirm(`¿Estás seguro de que querés eliminar "${product.name}"? Esta acción no se puede deshacer.`)) {
